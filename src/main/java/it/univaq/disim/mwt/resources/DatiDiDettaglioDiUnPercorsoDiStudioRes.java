...
@Path("dati_di_dettaglio_di_un_percorso_di_studio")
public class DatiDiDettaglioDiUnPercorsoDiStudioRes {

    @GET
    @Path("{cds_id: ([0-9]+)}/{aa_ord_id: ([0-9]+)}/{pds_id: ([0-9]+)}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getDatiDiDettaglioDiUnPercorsoDiStudio(@PathParam("cds_id") int cds_id, @PathParam("aa_ord_id") int aa_ord_id, @PathParam("pds_id") int pds_id, @QueryParam("lingua_iso6392_cod") String lingua_iso6392_cod) {
