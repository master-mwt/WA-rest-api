...
import it.univaq.disim.mwt.model.lists.PeriodoDidatticoAttivitaDidatticaList;
...
@Path("periodo_didattico_per_attivita_didattiche_offerte_in_un_percorso_di_studio")
public class PeriodoDidatticoPerAttivitaDidatticheOfferteInUnPercorsoDiStudioRes {

    @GET
    @Path("{cds_id: ([0-9]+)}/{aa_ord_id: ([0-9]+)}/{aa_off_id: ([0-9]+)}/{pds_id: ([0-9]+)}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getPeriodoDidatticoPerAttivitaDidatticheOfferteInUnPercorsoDiStudioRes(@PathParam("cds_id") int cds_id, @PathParam("aa_ord_id") int aa_ord_id, @PathParam("aa_off_id") int aa_off_id, @PathParam("pds_id") int pds_id, @QueryParam("ad_id") int ad_id, @QueryParam("lingua_iso6392_cod") String lingua_iso6392_cod) {
