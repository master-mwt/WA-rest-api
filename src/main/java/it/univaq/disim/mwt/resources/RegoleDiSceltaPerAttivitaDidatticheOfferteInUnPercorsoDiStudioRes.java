...
public class RegoleDiSceltaPerAttivitaDidatticheOfferteInUnPercorsoDiStudioRes {
    @GET
    @Path("cds_id/{cds_id: ([0-9]+)}/aa_ord_id/{aa_ord_id: ([0-9]+)}/aa_off_id/{aa_off_id: ([0-9]+)}/pds_id/{pds_id: ([0-9]+)}/anno_coorte/{anno_coorte: ([0-9]{4})}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getRegoleDiSceltaPerAttivitaDidatticheOfferteInUnPercorsoDiStudio(@PathParam("cds_id") String cds_id, @PathParam("aa_ord_id") String aa_ord_id, @PathParam("aa_off_id") String aa_off_id, @PathParam("pds_id") String pds_id, @PathParam("anno_coorte") String anno_coorte) {
